...
void path_to_field::initialize() {
    this->nh_ = ros::NodeHandle("~path_to_field");
    ...
    tf2_ros::TransformListener tf_listener(this->tf_buffer_);
}
...
void path_to_field::generate_attactor() {
    ...
    transform = this->tf_buffer_.lookupTransform(this->odom_frame_, this->frame_id_, ros::Time(0));
    geometry_msgs::Pose destination;
    tf2::doTransform(this->current_goal_.pose, destination, transform);

    float distance = std::sqrt(std::pow(destination.position.x,2) 
                    + std::pow(destination.position.y,2));
    double alpha = std::atan2(destination.position.y, destination.position.x);
